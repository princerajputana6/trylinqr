...
function shell(title, body) {
  ...
  <div style="background:#0f0f14;padding:32px 0;font-family:Inter,Arial,sans-serif">
    <div style="max-width:560px;margin:0 auto;background:#1c1c26;border-radius:16px;overflow:hidden">
      <div style="background:linear-gradient(135deg,#e63e62,#b01c41);padding:24px 32px">
...
export async function sendMail({ to, subject, html }) {
  ...
  return t.sendMail({
    from: `TryLinqr <${process.env.GMAIL_USER}>`,
    to,
    subject,
    html,
  });
  ...
  welcome: (name, verifyUrl) => ({
    ...
    html: shell(
      ...
      `<p>Thanks for joining TryLinqr. Please verify your email to activate your account.</p>
       <p><a href="${verifyUrl}" style="display:inline-block;background:#e63e62;color:#fff;padding:12px 24px;border-radius:10px;text-decoration:none;font-weight:600">Verify Email</a></p>
  ...
  adminApproved: (name) => ({
    ...
    html: shell(
      ...
      `<p>Your organizer account has been approved. You can now create and publish events.</p>
       <p><a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard" style="display:inline-block;background:#e63e62;color:#fff;padding:12px 24px;border-radius:10px;text-decoration:none;font-weight:600">Go to Dashboard</a></p>`
    ),
  }),
